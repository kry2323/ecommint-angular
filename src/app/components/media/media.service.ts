...
import {mediaConfig} from "./default-media.config";
...
export class MediaService {
  ...
  protected get sortedFormats(): { code: string; size: MediaFormatSize }[] {
    if (!this._sortedFormats && mediaConfig?.mediaFormats) {
      this._sortedFormats = Object.keys(mediaConfig.mediaFormats)
        .map((key) => ({
          code: key,
          size: mediaConfig.mediaFormats[key],
        }))
